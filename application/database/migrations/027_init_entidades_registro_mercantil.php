...
class Migration_Init_entidades_registro_mercantil extends CI_Migration {
        ...
        $string1 = read_file(APPPATH.'hooks/entidades_registro_mercantil.csv');
...
$datosentidad["$key"]['codigo_parent']= $sub;
  ...
  if($sub1!='')
        {   $this->db->where('codigo_registro',"$sub1");   
           $result = $this->db->get("$tb");
         $aux_union1_array = $result->result_array();
    
         if(count($aux_union1_array)>0)
         foreach ($datosentidad as $key => $value) 
        { 
...
$original_pa = $this->uuid->v5($aux_union1_array[0]['codigo_registro'],'8d3dc6d8-3a0d-4c03-8a04-1155445658f7');
if($parent_id!=$original_pa)
$parent_id = $original_pa;
    }
        ...
        $sub = str_replace("\r", "", $value['sub']); 
         ...
         {//$tipo = $aux_union_array[0]['nombre'];
          $tipo_id = $aux_union_array[0]['id'];
           ...
           $this->db->where('nombre','Registro Mercantil');   
           $result = $this->db->get("$tb");
         $aux_union_array = $result->result_array();
