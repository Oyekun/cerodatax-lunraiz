...
class Ion_auth_model extends CI_Model
{
	 ...
	 * @author    Mathew
	 */
	public function register($uuid,$identity, $password, $email, $additional_data = array(), $groups = array())
	{
		...
		$id = $uuid;//$this->db->insert_id($this->tables['users'] . '_id_seq');
	...
	public function increase_login_attempts($identity)
	{
		if ($this->config->item('track_login_attempts', 'ion_auth'))
		{
			...
			//$uuid = $this->uuid->v5($identity,'8d3dc6d8-3a0d-4c03-8a04-1155445658f7');
			 $data['date_created'] = $data['date_updated'] = date('Y-m-d H:i:s');
        $data['created_from_ip'] = $data['updated_from_ip'] = $data['ip_address'] = isset($_SERVER['REMOTE_ADDR']) ? $_SERVER['REMOTE_ADDR']: '127.0.0.1';
		//$this->db->set('id', $uuid); 
